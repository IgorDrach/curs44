...
def booking(request):
    if request.method == 'POST':
        ...
        total_cost_int = (adult_count * tour.price) + (child_count * (tour.price / 2))
        total_cost = int(total_cost_int)

    return render(request, 'app/booking.html', {'form': BookingForm(), 'tour_id': tour_id, 'adult_count': adult_count,'child_count': child_count,'total_cost': total_cost})

@login_required
def orders(request, tour_id,adult_count,child_count,total_cost):
    tour = get_object_or_404(Tour, id=tour_id)
